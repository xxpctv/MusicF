...
let siteKey = '';
let siteType = 0;
let cookie = 'PHPSESSID=e2jarvds8cdjik2crssfpsdbn1';
...
async function detail(id) {
    ...
    _.each(tabs, (tab, i) => {
        let from = $(tab).attr('data-dropdown-value');
        if (from.includes('夸克')) {
            return;
        }
...
async function play(flag, id, flags) {
    ...
    let playHtml = await request(HOST + '/okplayer/?url=' + playUrl);
...
function decryptUrl(html) {
    ...
    const md5Key = Crypto.MD5(keyStr + 'freeok@2024').toString();
...
async function search(wd, quick, pg) {
	const url = HOST + '/so1so/'+wd+'-------------.html';
    let html = await request(url, cookie);
	if (html.indexOf('系统安全验证') > 0) {
		await validCode(encodeURIComponent(url));
	}
    let $ = load(html);
...
async function validCode(refer) {
    try {
        //获取验证码的base64
        const res = await req(HOST + '/index.php/verify/index.html?', {
            buffer: 2,
            headers: {
                'User-Agent': UA,
                'Referer': refer,
        ...
        if(response['code'] === 200) {
            let checkRes = await req(HOST + '/index.php/ajax/verify_check?type=search&verify=' + response.content, {
				headers: {
                'User-Agent': UA,
                'Referer': refer,
                'Cookie': cookie
